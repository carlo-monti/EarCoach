...
- **Synth**: handles the musical part of the job. It uses the MidiDriver library [https://github.com/billthefarmer/mididriver](https://github.com/billthefarmer/mididriver) that simply send MIDI commands to the android internal general midi synth.
- **Vosk**: handles the speech-to-text job. It uses the Vosk library [https://alphacephei.com/vosk](https://alphacephei.com/vosk/) and it has been modified to accept only some words that are set with the setAcceptedKeywords() method.
